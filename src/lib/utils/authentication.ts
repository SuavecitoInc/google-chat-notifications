...
// User authentication
const CLIENT_SECRETS_FILE = '../../../client_secrets.json' as string;
const CLIENT_SECRETS = JSON.parse(
  (await readFile(new URL(CLIENT_SECRETS_FILE, import.meta.url))) as any
).web;
 ...
 */
export function createClientWithAppCredentials(): ChatServiceClient {
 ...
 */
export async function createClientWithUserCredentials(
  scopes: string[]
): Promise<ChatServiceClient> {
  // For more information on user authentication, see
  // https://developers.google.com/workspace/chat/authenticate-authorize-chat-user
  return new ChatServiceClient({
    authClient: (await getAuthenticatedUserOAuth2Client(scopes)) as any,
    scopes,
  });
}
 ...
 */
function getAuthenticatedUserOAuth2Client(
  scopes: string[]
